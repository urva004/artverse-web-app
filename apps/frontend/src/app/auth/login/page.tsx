  ...
  return (
    <div className="flex min-h-[calc(100vh-4rem)] items-center justify-center px-4 py-12">
      ...
      <div className="absolute inset-0 -z-10 overflow-hidden">
        <div className="absolute -left-40 top-20 h-80 w-80 rounded-full bg-[var(--color-accent)] opacity-5 blur-[100px]" />
        <div className="absolute -right-20 bottom-20 h-60 w-60 rounded-full bg-[var(--color-teal)] opacity-5 blur-[80px]" />
      </div>
        ...
        <div className="rounded-card border border-[var(--color-border)] bg-[var(--color-card)] p-8">
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="flex h-11 w-full items-center justify-center gap-2 rounded-input bg-gradient-to-r from-[var(--color-accent)] to-[var(--color-accent-2)] text-sm font-semibold text-white shadow-glow transition-all hover:shadow-glow-md disabled:opacity-50"
            >
              {isLoading ? (
